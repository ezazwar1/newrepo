...
(function() {
  angular.module('ionic.rating', []).constant('ratingConfig', {
    ...
    return {
      ...
      template: '<ul class="rating" ng-mouseleave="reset()" ng-keydown="onKeydown($event)">' + '<li ng-repeat="r in range track by $index" aria-label="rate {{$index + 1}} star" ng-click="rate($index + 1)"><i role="button" aria-label="rate {{$index + 1}} star" class="icon" ng-class="$index < value && (r.stateOn || \'ion-ios-star\') || (r.stateOff || \'ion-ios-star grey\')"></i></li>' + '</ul>',
      replace: true,
