...
.controller('DashCtrl', function($scope, $rootScope, $interval, $ionicModal, $ionicPopup, Utils, $state, Timer, Share, History, Variables) {
    ...
    $scope.Metric       = Variables.getMetric();
    $scope.Speed = 
    ...
    $scope.changeMetric = function() {
        $scope.Metric = Variables.changeMetric();
        updateUserSpeed();
    };
    ...
    function updateUserSpeed() {
        ...
        if($scope.Metric == $scope.Metric["options"].us) {
    ...
    function setCustomSpeed(index, initBoolean){
        ...
        if(index == $scope.CustomSpeeds.length-1) {
            ...
            // set temp speed
            if($scope.Metric.choice == $scope.Metric["options"].us) {
